...
namespace appChatServer.Message
{
    ...
    /// 서버 등록 RQ
    /// </summary>
    public class Msg_Svr_RegisterAtChat_RQ : IMessage
    {
        ServerSession m_session = null;
        Packet m_recvPacket = null;

        public Msg_Svr_RegisterAtChat_RQ(appServer server, SessionBase session, Packet packet)
        {
            m_server = server;
            m_recvPacket = packet;
        ...
        {
            using (var recvfmProtocol = new PT_Server_RegisterAtChat_RQ())
            {
                recvfmProtocol.Deserialize(m_recvPacket);

                using (var sendfmProtocol = new PT_Server_RegisterAtChat_RS())
                {
