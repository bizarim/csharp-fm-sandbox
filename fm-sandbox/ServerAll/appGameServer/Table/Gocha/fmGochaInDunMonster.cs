...
{
    public class fmGochaInDunMonster
    {
        ...
        public fmGochaInDunMonster(Random ran)
        {
            m_random = ran;
        }

        public void Add(fmDataInDun data)
        {
            int cnt = data.m_nAppearMon.Length;
            for (int i = 0; i < cnt; ++i)
            {
                int roll = 100 * m_nRate * data.m_nArrAppearRateMon[i];

                fmGochaNode node = new fmGochaNode();
                node.m_fmData = null;

                node.m_nGochaValue = data.m_nAppearMon[i];
